...
internal sealed class NodeRecord
{
    ...
    /// <summary>Hardware snapshot at the most recent join or
    /// heartbeat.</summary>
    ...
    /// <summary>UTC time of the most recent heartbeat. Used by
    /// the future health monitor to flip nodes to
    /// <c>Offline</c>.</summary>
    public DateTimeOffset LastHeartbeatAt { get; set; }

    /// <summary>Count of workloads the node reports running.
    /// Used by the future scheduler as a load input.</summary>
    public int RunningVmCount { get; set; }

    /// <summary>Join-token hash. v0.1 stores the token as-is (no
    /// hashing). Real impl uses SHA-256.</summary>
    public required string JoinTokenHash { get; init; }

    /// <summary>Optional friendly name (set via heartbeat, not
    /// registration). Future impl: stable across restarts.</summary>
