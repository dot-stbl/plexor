...
/// <remarks>
///     <para>
///         <b>Schema-per-module.</b> Subclasses override
...
///     </para>
///     <para>
///         <b>Tracking by default.</b> Subclasses use the standard
///         EF Core <c>ChangeTracker</c> — read-only query paths opt out
///         per-query with <c>.AsNoTracking()</c>, never globally.
///     </para>
///     <para>
...
/// </remarks>
/// <remarks>
///     Constructs a DbContext with the supplied options and optional
///     module-specific OnModelCreating hook (rare — prefer overriding
///     <see cref="OnModelCreating" /> in the subclass).
/// </remarks>
/// <param name="options">Standard EF Core options bag.</param>
/// <param name="onModelCreatingHook">
///     Optional hook for tests that need
///     to inject models programmatically (production code should override
///     the virtual method).
/// </param>
public abstract class PlexorDbContext(
    DbContextOptions options,
    Action<ModelBuilder>? onModelCreatingHook = null) : DbContext(options)
{
    /// <summary>
