...
// ============================================================================
// LibvirtRunner — thin wrapper around the `virsh` CLI. Lives in
// Plexor.Shared.Network (host-side, used by LibvirtNetworkProvider)
// — the NodeAgent has its own LibvirtRunner in Plexor.NodeAgent
// for symmetry. Same shape; independent process management so the
// two runtimes don't share a connection.
//
...
public static class LibvirtRunner
{
    ...
    /// </summary>
    /// <param name="uri">
    ///     Libvirt URI (e.g. <c>qemu:///system</c>). The runner
    ///     prefixes <c>-c</c> and the URI to the command line so
    ///     the provider doesn't have to.
    /// </param>
    /// <param name="args">
    ///     Virsh command + flags (e.g. <c>"net-list --all"</c>,
    ///     <c>"net-define /etc/libvirt/qemu/networks/foo.xml"</c>).
