...
/// </summary>
/// <param name="DiskBytes">Root filesystem capacity by default; the
/// agent exposes a single number for v0.1 and the control plane treats
/// it as advisory (not a reservation limit).</param>
...
public abstract record WorkloadKind
{
    ...
    public sealed record Vm() : WorkloadKind { /// <summary>Wire name (see <see cref="CommandType.Name" />).</summary>
 public override string Name => "vm"; }

    /// <summary>LXC system container via libvirt (uri <c>lxc:///</c>).</summary>
    public sealed record Lxc() : WorkloadKind { /// <summary>Wire name (see <see cref="CommandType.Name" />).</summary>
 public override string Name => "lxc"; }

    /// <summary>Kubernetes pod scheduled by a k3s / upstream k8s API.</summary>
    public sealed record K8sPod() : WorkloadKind { /// <summary>Wire name (see <see cref="CommandType.Name" />).</summary>
 public override string Name => "k8s.pod"; }

    /// <summary>Generic container (podman / docker) on the host.</summary>
    public sealed record Container() : WorkloadKind { /// <summary>Wire name (see <see cref="CommandType.Name" />).</summary>
...
/// </summary>
/// <param name="Config">Provider-specific JSON. Each provider defines
/// its own schema; the shared contract is opaque JSON.</param>
...
public abstract record CommandType
{
    ...
    public sealed record WorkloadCreate() : CommandType { /// <summary>Wire name (see <see cref="CommandType.Name" />).</summary>
 public override string Name => "workload.create"; }

    /// <summary><c>workload.start</c> — boot a previously provisioned workload.</summary>
    public sealed record WorkloadStart() : CommandType { /// <summary>Wire name (see <see cref="CommandType.Name" />).</summary>
 public override string Name => "workload.start"; }

    /// <summary><c>workload.stop</c> — gracefully shut down a running workload.</summary>
    public sealed record WorkloadStop() : CommandType { /// <summary>Wire name (see <see cref="CommandType.Name" />).</summary>
 public override string Name => "workload.stop"; }

    /// <summary><c>workload.delete</c> — remove a workload and its backing storage.</summary>
    public sealed record WorkloadDelete() : CommandType { /// <summary>Wire name (see <see cref="CommandType.Name" />).</summary>
...
public enum CommandResultStatus
{
    ...
    /// <summary>Command failed; <c>ErrorMessage</c> on the result envelope
    /// is populated with a one-line human-readable detail.</summary>
    Failed,
}
...
/// </summary>
/// <param name="SentAt">UTC time the heartbeat was sent (so the
/// control plane can spot clock-skew across many nodes).</param>
