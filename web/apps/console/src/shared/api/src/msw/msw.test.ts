...
describe('MSW handler coverage — admin-page mount surface', () => {
  ...
  // ───────────────────────── Dashboard widgets ─────────────────────────
  // The `/` route renders 4 widgets: VmStatusDonut, FleetTotalsStats,
  // QuotaUsageBars, AuditTimelineList. Each consumes one of these
  // endpoints; the assertions verify that the hand-curated fixtures
  // (added in feature/more-mocks-ddd) actually return data the widgets
  // can render — random faker strings / ints would render as "???".

  it('dashboard: /vms returns ≥ 2 distinct statuses so the donut renders', async () => {
    const res = await fetch('/vms');
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.items.length).toBeGreaterThanOrEqual(2);
    const statuses = new Set(body.items.map((vm: { status: string }) => vm.status));
    expect(statuses.size).toBeGreaterThanOrEqual(2);
  });

  it('dashboard: /vms items have numeric vcpu/ram/disk so fleet totals are real', async () => {
    const res = await fetch('/vms');
    const body = await res.json();
    const sample = body.items[0] as { vcpu: unknown; ramGb: unknown; diskGb: unknown };
    expect(typeof sample.vcpu).toBe('number');
    expect(typeof sample.ramGb).toBe('number');
    expect(typeof sample.diskGb).toBe('number');
  });

  it('dashboard: /quotas/usage returns real metric names with used < limit', async () => {
    const res = await fetch('/quotas/usage');
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.length).toBeGreaterThan(0);
    for (const entry of body as Array<{ metric: string; used: number; limit: number }>) {
      ...
      expect(entry.used).toBeLessThanOrEqual(entry.limit);
    }
  });

  it('dashboard: /audit returns dot.case action verbs in descending order', async () => {
    const res = await fetch('/audit');
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.length).toBeGreaterThan(0);
    for (const entry of body as Array<{ action: string; occurredAt: string }>) {
      expect(entry.action).toMatch(/^[a-z][a-z0-9_]*(\.[a-z][a-z0-9_]*)+$/);
    }
    const timestamps = body.map((entry: { occurredAt: string }) => new Date(entry.occurredAt).getTime());
