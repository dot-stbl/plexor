...
describe('MSW handler coverage — admin-page mount surface', () => {
  ...
  // ───────────────────────── Login (1) ─────────────────────────
  // Phase 4.6 endpoint — generated by kubb from /auth/login (added in
  // contracts/plexor.openapi.yaml, regen produced
  // shared/api/src/{client,types,schemas,fixtures,msw}/postAuthLogin*).
  // The dev:mock worker returns 200 + a kubb-shaped login response so
  // the login page's postAuthLogin() always resolves with a usable mock
  // triple.
