...
import { DotsSixVertical, Check, CaretUp, CaretDown } from '@phosphor-icons/react'
  ...
  return (
    ...
    >
      ...
      <SelectPrimitive.Icon
        render={
          <DotsSixVertical strokeWidth={2} className="pointer-events-none size-3.5 text-muted-foreground"  />
        }
      />
    </SelectPrimitive.Trigger>
  )
...
function SelectItem({
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "relative flex min-h-7 w-full cursor-default items-center justify-between gap-2 rounded-md px-2 py-1 text-xs/relaxed outline-hidden select-none focus:bg-accent focus:text-accent-foreground not-data-[variant=destructive]:focus:**:text-accent-foreground data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-3.5",
        className
      )}
      {...props}
    >
      ...
      <SelectPrimitive.ItemIndicator
        render={
          <Check strokeWidth={2} className="pointer-events-none size-3.5 text-foreground" />
        }
      />
    </SelectPrimitive.Item>
  )
