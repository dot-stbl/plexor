...
/**
 * Создание БД — отдельная СТРАНИЦА (не модалка). Движок (предвыбран из
 * ?engine), параметры + бэкапы, runtime-picker (valid ∩ available), review
...
function CreateDatabasePage() {
  ...
  return (
    <main data-od-id="databases-new">
      <Breadcrumb className="mx-auto w-full max-w-3xl px-6 pt-6 lg:px-8">
        <BreadcrumbList>
          <BreadcrumbItem>
            <BreadcrumbLink render={<Link to="/databases" />}>Платформа данных</BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbLink render={<Link to="/databases/new" />}>Новая БД</BreadcrumbLink>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      ...
      <PageHeader
        ...
        actions={
          <Button variant="ghost" nativeButton={false} render={<Link to="/databases" />}>
            <ArrowLeft />
            Назад
          </Button>
        }
      ...
      <div className="mx-auto w-full max-w-3xl space-y-3 px-6 py-6 lg:px-8">
          ...
          <CardContent className="p-4">
            <FieldGroup>
              <Field>
                ...
                />
                <FieldDescription>Используется как имя инстанса и в DNS.</FieldDescription>
              </Field>
        ...
        <div className="flex items-center justify-between">
          <Button variant="outline" nativeButton={false} render={<Link to="/databases" />}>
            Отмена
          </Button>
