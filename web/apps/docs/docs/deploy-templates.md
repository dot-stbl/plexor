# Deployment templates — `@plexor/docs`

Copy-and-edit templates for hosting the static `dist/` from
`bun run build:docs`. **Not** committed production config — once the
...
COPY --from=build /repo/web/apps/docs/dist /usr/share/nginx/html
