...
> Plexor public docs site. Sibling of `@plexor/console` in the
> `web/` monorepo, sharing `@plexor/ui` for the Plexor DS tokens and
> primitives. **Read this before touching anything in `web/apps/docs/`.**

## 1. What this app is

Static docs site that serves both the marketing landing (`/`) and the
operator-facing documentation (`/docs/*`). Same Plexor DS tokens as
`@plexor/console`, same theme presets, same icon set, same theming
primitive. Lives at `web/apps/docs/`; runs as a Vite SPA on dev port
...
| Script | What it does |
|---|---|
| `bun run dev` (alias `dev:docs` at root) | Vite dev on **port 17101** |
| `bun run build` (alias `build:docs`) | Static build to `dist/` |
| `bun run preview` (alias `preview:docs`) | Vite preview on **port 17111** |
| `bun run typecheck` (alias `typecheck:docs`) | `tsc --noEmit` |
| `bun run lint` (alias `lint:docs`) | eslint `--max-warnings 0` |
| `bun run test` (alias `test:docs`) | `vitest run` |
...
- `web/docs/PORTS.md` — dev port pool (17100–17199); docs uses
...
- `../../.agents/rules/process/commit-format.md` — commit message
  format (`[.stbl](feat/fe/docs): …`).
- `~/.agents/rules/process/agent-runtime-safety.md` — no dev servers.

## 11. Don't duplicate

This file is the **docs-app-specific** layer. Project-wide rules
