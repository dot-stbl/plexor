# Spec delta: network (phase-4-5-d-storage-network)

This file is the proposed state of
`openspec/specs/network/spec.md` after the change
...
`openspec/specs/network/spec.md` under `## Requirements`,
...
- `TargetWorkloadIds : IReadOnlyList<Guid>` — list of
  ...
  (`ef-owned-types.md`).
- `Status : LoadBalancerStatus` — `Provisioning` | `Active`
  | `Deleting`.
- `CreatedAt : DateTimeOffset`, `UpdatedAt :
  DateTimeOffset`.
...
1. Validates the request (FluentValidation: `address` is a
   valid IPv4 literal; `nodeId` if present references a
...
1. Validates the request (FluentValidation: `name` non-empty
   ≤ 64 chars; `port` 1–65535; `algorithm` is one of
   `round-robin` / `least-conn`; every
...
(`Plexor.Modules.Network.Infrastructure`) implements the
interface by querying `network.floating_ips` /
...
- `GET /api/v1/network/load-balancers/{id}` (`network.read`).
...
`network`. The `InitNetwork` migration is the only network
migration in v0.1.
...
`Plexor.Modules.Network.Domain.Entities.LoadBalancer`
(schema `network.load_balancers`). Fields as listed above.

### Value Objects and Exceptions

- `FloatingIpId` (record struct) — typed wrapper, prefix
  `fip_<UUIDv7>`.
- `LoadBalancerId` (record struct) — typed wrapper, prefix
  `lb_<UUIDv7>`.
