...
Plexor's storage module owns two resource types:

1. **Volumes** — block-storage volumes attached to Plexor
   nodes.
2. **Buckets** — S3-compatible object-storage buckets.

The module is the consumer of three quota catalog keys
shipped by Phase 4.5 (`storage.volumes.count`,
`storage.volumes.gb`, …) and exposes REST endpoints for
volume + bucket CRUD. Quota reservation happens inside the
resource-create transaction so a failed INSERT rolls back
the reservation.

This capability is owned by `Plexor.Modules.Storage`.
Schema name in SQL and migrations: `storage`. C# concept
names: `Volume`, `Bucket`, `VolumeId`, `BucketId`.
...
- `NodeId : NodeId?` — assigned node id; null until a
  node claims the volume.
...
`EfStorageQuotaReader`
(`Plexor.Modules.Storage.Infrastructure`) implements
the interface by querying `storage.volumes` directly with
the same tenant filter as every other Plexor read. The
interface is the seam that keeps the quota system from
leaking its internal `QuotaUsage` rows.
...
- `POST /api/v1/storage/volumes` (`storage.create`) —
  create.
- `GET /api/v1/storage/volumes?cursor=&limit=`
  (`storage.read`) — list with cursor pagination.
...
The system SHALL expose the same five endpoints for
buckets:
